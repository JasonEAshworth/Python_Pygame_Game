...
class ClientInfo(object):
    ...
    def __del__(self):
        """ Destructor """
        del ClientInfo.ipIndex[self.addr]




class Server(connection.ConnectionListener):
...
###############################################################################
# Callback functions called by the connection (since we're registered         # 
...
###############################################################################                                         
    def onNewMessage(self, m, addr):
        ...
        self.pendingMessagesSemaphore.acquire()
        self.pendingMessages.append([cinfo.ID, m, cinfo.addr])
        self.pendingMessagesSemaphore.release()


    def onThreadExit(self):
        """ Called just before the connection thread shuts down. """
        self.logMessage("Connection thread is shutting down", 0)

    def onAbnormalDisconnect(self):
        """ Called if the other end of the socket abnormally terminates. """
        self.logMessage("Connection thread is *unexpectedly* shutting down", 0)
...
###############################################################################
    ...
    def updateDBase(self, playerObj):
        ...
        if playerObj.uname in self.dbase:
            ...
            self.logMessage("User '" + playerObj.uname + \
...
###############################################################################           
    ...
    def logMessage(self, s, lvl):
        """ Logs a messages.  By default, this just prints.  If re-defined 
            in a dervied class it could 'pipe' information to a GUI. """
...
###############################################################################
    ...
    def __processNewAccountMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing NEW_ACCOUNT msg", 1)
        me = msg.split(":")
        if len(me) == 3 and len(me[1]) > 6 and me[1].find("=") == 5 and \
                         len(me[2]) > 5 and me[2].find("=") == 4:
            self.logMessage("Creating new account", 2)
            ...
            if uname in self.dbase:
                self.logMessage("Sending NEW_ACCOUNT_ACK - rejected", 1)
                self.sendMessage("<NEW_ACCOUNT_ACK:accepted=F>", ID, True)
            else:
                self.logMessage("Sending NEW_ACCOUNT_ACK - accepted", 1)
                self.sendMessage("<NEW_ACCOUNT_ACK:accepted=T>", ID, True)
                tempPlayer = player.Player(uname)
                self.addToDBase(tempPlayer, passW)
                self.saveDBase()
        else:
            self.logMessage("Corrupt NEW_ACCOUNT message from client #" + \
                                  str(ID), 1)

    def __processLoginMsg(self, msg, ID, addr):
        """ Process a LOGIN message.  Checks the database for a uname/pass 
            that matches.  If found, returns the player's state in the 
            LOGIN_ACK message. """
        self.logMessage("Processing LOGIN msg", 1)
        me = msg.split(":")
        if len(me) == 3 and len(me[1]) > 6 and me[1].find("=") == 5 and \
                               len(me[2]) > 5 and me[2].find("=") == 4:
            ...
            if not correct:
                ...
                # Create the player
                self.logMessage("Creating new player (" + str(ID) + ")", 1)
                self.players[ID] = player.Player(uname)
                self.logMessage("Deserialize String = '" + serialStr + "'", 6)
            ...
            self.logMessage("Corrupt LOGIN message from client #" + str(ID), 1)
    ...
    def __processUpdateMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing UDPATE msg '" + str(msg) + "' from ID#" + \
                                    str(ID), 3)
        serialStr = msg[7:]
        if ID in self.players:
           self.logMessage("Updating Player#" + str(ID) + " with '" + \
                        serialStr + "'", 3)
           self.players[ID].deserialize(serialStr)
           self.logMessage("Player#" + str(ID) + "=" + str(self.players[ID]), 3)
        ...
        for k in self.players:
            if k != ID:
                self.logMessage("\tSending '" + msg + " to (" + str(k) + ")", 3)
    ...
    def __processFullUpdateMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing FULL_UPDATE msg", 3)
    ...
    def __processPlayerLeftMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing PLAYER_LEFT msg", 1)
        me = msg.split(":")
        for k in self.players:
            if k != ID:
    ...
    def __processQuitMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing QUIT msg", 1)
        self.sendMessage("<QUIT_ACK>", ID, True)
        me = msg.split(":")
        playersToDelete = []
        if ID in self.players:
            self.logMessage("Deleting Player#" + str(ID), 1)
        ...
        # other players then
        self.onNewMessage("PLAYER_LEFT", self.clients[ID].addr)
    ...
    def __processPingMsg(self, msg, ID, addr):
        ...
        self.logMessage("Processing PING message", 5)
...
###############################################################################
    def sendHeartBeat(self, dT):
        ...
        self.logMessage("Sending heartbeat", 4)
        m = "<FULL_UPDATE_ACK"
        for k in self.players:
            m += ":ID=" + str(k) + ":"
            p = self.players[k]
...
###############################################################################
    def updateGUI(self):
        ...
        self.screen.blit(self.font.render("SERVER   " + \
                          ...
                          str(self.serverPort), False, (255,255,255)), (0,0))
