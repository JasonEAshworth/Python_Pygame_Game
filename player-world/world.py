...
class Zone(object):
	...
	def getTile(self,x,y):
		return self.tlist[int(x // self.tileSize)][int(y // self.tileSize)]
	...
	def collision(self, objectPos, enemy = False):
		tmpPos = self.playerPos[0]//self.tileSize,self.playerPos[1] // self.tileSize
			...
			for i in self.eList:
				while z < int(self.eList[p][0]):
					if self.eList[p][1] == 'Cow':
						health = 10
						enemy = CamCharacterClass.Cow([self.enemySpawn[z][0],self.enemySpawn[z][1]], 100/1000.0, health)
						self.enemyList.append(enemy)
					elif self.eList[p][1] == 'Wolf':
						health = 50
						enemy = CamCharacterClass.Wolf([self.enemySpawn[z][0],self.enemySpawn[z][1]], 100/1000.0, health)
						self.enemyList.append(enemy)
					z += 1
				p += 1
				z = 0
...
if __name__ == '__main__':
	...
	pygame.display.init()
	pygame.font.init()
	screen = pygame.display.set_mode(((area.winWidth+300),area.winHeight))
	cameraSpeed = 8
	...
	while not done:
		...
		# Update variables
		area.cameraPos[0] = area.playerPos[0] - area.winWidth/2
		area.cameraPos[1] = area.playerPos[1] - area.winHeight/2
...
##			n +=1
##		if n < area.totalNumEnemies:
##			z = 0
##			p = 0
##			for i in area.eList:
##				while z < int(area.eList[p][0]):
##					print(p)
##					if area.eList[p][1] == 'Cow':
##						enemy = CamCharacterClass.Cow([random.randint(500,area.world_pixelWidth-500),random.randint(500,area.world_pixelHeight - 500)], health)
##						area.enemyList.append(enemy)
##					elif area.eList[p][1] == 'Wolf':
##						enemy = CamCharacterClass.Wolf([random.randint(500,500),random.randint(500,500)], health)
##						area.enemyList.append(enemy)
##					z += 1
##				p += 1
##				z = 0
		...
		# Draw the level
		area.render(screen)
		pygame.draw.circle(screen, (128,128,128), (int(area.playerPos[0] - area.cameraPos[0]), int(area.playerPos[1] - area.cameraPos[1])), 10, 0)
		area.OverLayRender(screen)
		# Check each tile around player for collision checks
		area.tileCheck(area.playerPos[1],area.playerPos[0])

		#Collision detection
		area.collision(area.playerPos)
		...
		if pressedList[pygame.K_LEFT]:
			area.playerPos[0] -= cameraSpeed
		if pressedList[pygame.K_RIGHT]:
			area.playerPos[0] += cameraSpeed
		if pressedList[pygame.K_UP]:
			area.playerPos[1] -= cameraSpeed
		if pressedList[pygame.K_DOWN]:
			area.playerPos[1] += cameraSpeed
		pygame.display.flip()
