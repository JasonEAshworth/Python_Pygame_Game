...
while not done:
	...
	enemy.checkIfHit(area.cameraPos, test.bullets) # Hit Detection
	enemy.playerHitCheck(area.cameraPos, test)     # Hit Detection
	#enemy #2
	enemy2.move(p_pos[0], p_pos[1], dtime, area.cameraPos)
	enemy2.draw(window, dtime, None, area.cameraPos)
	enemy2.checkIfHit(area.cameraPos,test.bullets) # Hit Detection
	enemy2.playerHitCheck(area.cameraPos, test)    # Hit Detection
	#call player class
